...
NumPy provides {\tt absolute}, which computes
...
amplitude ratio is approximately 9.  The frequency ratio of the
...
Repeat for a range of values of $i$.  How does the sound change as
you remove harmonics?
\end{exercise}
...
{\tt dps = PI2 * freqs * dts}.  In math notation:
...
the first {\tt i} terms from {\tt dps}; that is, the total
...
This chapter is about noise in the second sense.
...
% TODO: why power here rather than amplitude?
...
{\tt self.power} is an array of power for each frequency.
...
In a one-dimensional random walk, the particle moves left or right
...
generate an uncorrelated signal and then compute its discrete integral.
...
At \url{http://www.coindesk.com/price/} you can download the daily
...
$\rho$.  For two variables that contain $N$ values, $x_i$ and $y_i$:

\[ \rho = \frac{ \sum_i (x_i - \mu_x) (y_i - \mu_y)}{N \sigma_x \sigma_y} \]

Where $\mu_x$ and $\mu_y$ are the means of $x_i$ and $y_i$, and
...
statistics, but less commonly relevant for signal processing, so I
won't say more about them here.

In the context of signal processing, we are often working with
unbiased waves, where the mean is 0, and normalized waves, where the
...
\begin{verbatim}
>>> corr_matrix = numpy.corrcoef(wave1.ys, wave2.ys)
[[ 1.    0.54]
 [ 0.54  1.  ]]
\end{verbatim}

The first element of this array is the correlation of {\tt wave1}
...
The off-diagonal elements contain the values we're interested in,
...
Many signals represent measurements of quantities that vary in
...
{\tt lag} is the integer number of places to shift the waves,
...
integers from 0 to half of the length of the wave; {\tt corrs}
...
Figure~\ref{fig.autocorr6} shows the result.  There is a clear peak near
...
On segment starts at 0.2 seconds; the other starts 0.0023 seconds
later.  Figure~\ref{fig.autocorr7} shows the result.  The segments
are very similar, and their correlation is 0.94.  This result suggests
...
We've been using the discrete Fourier transform (DFT) since Chapter 1,
but I haven't explained how it works.  Now is the time.
...
\item As in the previous chapter, I'll start with the synthesis
...
is a function that operates on integers; the natural definition of
factorial of $n$ is the product of all integers from 1 to $n$.

Being a troublemaker, you might wonder how to compute the factorial
of a non-integer like 3.5.  Since the natural definition doesn't apply,
you might look for other ways to compute the factorial function,
ways that would work with non-integers.
...
Exponentiation can be written as a power series:
...
\[ e^{x + i\phi} = e^x e^{i\phi} = A e^{i\phi} \]
...
complex object.  Note that Python uses {\tt j} to represent
the imaginary unit, rather than {\tt i}.

When the argument to
{\tt numpy.exp} is imaginary or complex, the result is a complex
number; specifically, a {\tt numpy.complex128}, which
is represented by two 64-bit floating-point numbers.
...
that is, when $\phi(t) = 2 \pi f t$, the result is a {\bf complex
sinusoid}:
...
to the synthesis problem: the frequency and amplitude of
...
real signals: they represent quantities that vary in time; in
the case of sound signal, they represent changes in air pressure.
...
question.  The best I can offer is two bad answers:

% mathematical abstraction useful for intermediate results

% two real signals

A segment of this signal is shown in Figure~\ref{fig.dft1}.
