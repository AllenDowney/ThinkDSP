...
The code for this chapter is in {\tt harmonics.py}, which is in the
repository for this book (see Section~\ref{code}).
...
As we saw in Section~\ref{???}, we can also express the synthesis
...
When we multiply {\tt M} by the amplitudes, the elements of the
result correspond to the {\tt ts}; each element is the sum of
...
% unpacking the frequency components of the DFT, negative frequencies
