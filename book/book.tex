...
Figure~\ref{fig.aliasing-3} shows the result.  I plotted the
...
You can listen to these examples in {\tt chap03.ipynb}.  If you have a
musical ear, it might sound more like music than the linear chirp.
...
assumption is often not true, which creates problems.
...
Inside the while loop, we select a slice from the wave and multiply
by the window; then we construct a Spectrum
object and add it to \verb"spec_map".  The nominal time of
each segment is the midpoint.
...
Solutions to these exercises are in {\tt chap06soln.ipynb}.
...
The code for this chapter is in {\tt sampling.py}, which is in the
repository for this book (see Section~\ref{code}).

You can listen to the examples in an IPython notebook, {\tt chap11.ipynb}, 
which is also in the repository, and you can view it at
at \url{http://tinyurl.com/thinkdsp11}.
