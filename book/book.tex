...
This signal approximates a sinusoid, which means it has the same shape as
...
cosine signal with {\tt offset=0} starts at $\cos 0$, which is 1.
With {\tt offset=pi/2} it starts at $\cos \pi/2$, which is 0.  A sine
signal with {\tt offset=0} also starts at 0.  In fact, a cosine signal
with {\tt offset=pi/2} is identical to a sine signal with {\tt
  offset=0}.
...
The {\tt play} method write the wave to a file and then plays it.
...
In the previous chapter you used the classes defined in {\tt thinkdsp.py};
in this chapter I explain more about how they work.

I present several new waveforms and we will look and their
spectrums to understand their {\bf harmonic structure}, which is
the set of sinusoids they comprise.

Finally, I introduce one of the most interesting phenomena in
...
\section{Implementing Sinusoids}

The {\tt thinkdsp} module, which you saw in the previous chapter,
provides {\tt CosSignal} and {SinSignal}, which create cosine
and sine signals: 
...
\item {\tt offset}: where in its period the signal starts;
...
The parent class of {\tt Sinusoid}, {\tt Signal}, provides
\verb"make_wave", which evaluates the signal at a sequence of
points in time and returns a Wave.
...
class Spectrum(object):
    ...
    def __init__(self, hs, framerate):
        ...
        n = len(hs)
        f_max = framerate / 2.0
        self.fs = np.linspace(0, f_max, n)

        self.amps = np.absolute(self.hs)
\end{verbatim}
...
{\tt Spectrum} provides {\tt plot}, which plots the amplitudes
versus the frequencies:
...
and Spectrum classes work, but I have not explained how the
Discrete Fourier Transform works.  That will take a few more chapters.
...
%\begin{exercise}
% TODO: add an exercise about plotting phases
%\end{exercise}
...
%
{\tt np.cumsum} computes the cumulative sum, which you can think
of as a simple kind of integration.  If you know calculus, explanations
like that might help; otherwise you can ignore them.
...
Here's an example that creates and plays a chirp from 220 to 880 Hz, which
...
You can run this example in {\tt chap03.ipynb}.  If you have a musical
ear, it might sound more like music than the linear chirp.
...
{\tt np.hamming} computes the Hamming window with the given number
...
segments from the wave.  {\tt step} is the offset between
...
{\tt plot} uses {\tt times}, which returns the midpoint of each
...
for each frequency.  The {\tt for} loop iterates through the {\tt ts}
...
The following example makes generates UU noise with duration 0.5
...
To see the shape of the spectrum more clearly, we can plot power density
...
The result is in Figure~\ref{fig.redline}.  The relationship between
...
at $f=0$ as a special case, partly to avoid dividing by 0, partly
...
But before we get to autocorrelation, we'll start with correlation.
...
the interface to {\tt np.corrcoef}.
...
\begin{exercise}
SciPy provides an implementation of DCT in
{\tt fftpack}.  Read the documentation of {\tt scipy.fftpack.dct}
and run it on one of the examples in this chapter.  Confirm that
the results are the same as my implementation.
\end{exercise}


\begin{exercise}
In this chapter I claim that {\tt analyze1} takes time proportional
to $n^2$ and {\tt analyze2} takes time proportional to $n^2$.  To
...
difference is that it uses {\tt np.exp} instead of
{\tt np.sin}.
...
The simplest solution is to instantiate {\tt ComplexSinusoid}
...
is an array of reals, multiplying by {\tt np.exp(1.5j)} yields
...
We are almost done.  This function computes something very
...
With {\tt mode='valid'}, {\tt np.convolve} returns values
